import java.util.*;

class RecentlyPlayedStore {
    private int capacity;
    private Map<String, LinkedList<String>> userPlaylists;
    private LinkedList<String> songOrder;

    public RecentlyPlayedStore(int initialCapacity) {
        this.capacity = initialCapacity;
        this.userPlaylists = new HashMap<>();
        this.songOrder = new LinkedList<>();
    }

    public void playSong(String user, String song) {
        userPlaylists.putIfAbsent(user, new LinkedList<>());
        LinkedList<String> userPlaylist = userPlaylists.get(user);

        if (userPlaylist.contains(song)) {
            userPlaylist.remove(song);
        }
        userPlaylist.add(song);

        if (songOrder.contains(song)) {
            songOrder.remove(song);
        }
        songOrder.add(song);

        if (songOrder.size() > capacity) {
            String removedSong = songOrder.poll();
            for (LinkedList<String> playlist : userPlaylists.values()) {
                playlist.remove(removedSong);
            }
        }
    }

    public List<String> getRecentlyPlayed(String user) {
        return userPlaylists.getOrDefault(user, new LinkedList<>());
    }
}

public class Main {
    public static void main(String[] args) {
        RecentlyPlayedStore store = new RecentlyPlayedStore(3);

        store.playSong("user1", "S1");
        store.playSong("user1", "S2");
        store.playSong("user1", "S3");
        System.out.println(store.getRecentlyPlayed("user1"));  // Output: [S1, S2, S3]

        store.playSong("user1", "S4");
        System.out.println(store.getRecentlyPlayed("user1"));  // Output: [S2, S3, S4]

        store.playSong("user1", "S2");
        System.out.println(store.getRecentlyPlayed("user1"));  // Output: [S3, S4, S2]

        store.playSong("user1", "S1");
        System.out.println(store.getRecentlyPlayed("user1"));  // Output: [S4, S2, S1]
    }
}
